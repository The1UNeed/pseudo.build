...
import { compilePseudocode, parseSource, SYNTAX_OPTIONS } from "./index";
...
describe("OUTPUT lists", () => {
  it("keeps reading values after one that starts with a parenthesis", () => {
    const result = compilePseudocode({ source: "OUTPUT (1 + 2) * 3, 4", filename: "main.pseudo", strict: true });
