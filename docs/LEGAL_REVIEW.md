...
| Analytics only on Vercel, cookieless | `apps/web/src/app/[locale]/layout.tsx` | Correct |
| Monaco served from own origin, CSP/HSTS set | `apps/web/next.config.ts`, `monacoLocalLoader.ts` | Correct |
...
3. **No international-transfer notice.** All providers are US companies. Added a recipient table, the EU/UK
   ...
   (recipient, purpose, method, categories, how to exercise rights, separate consent, withdrawal).
...
10. **Security page:** added the email reporting channel (GitHub private reporting cannot work while the
    repository is private), the incident-notification section, and the account-deletion data flow.
...
- Set `CLERK_WEBHOOK_SECRET` on the Convex deployment and enable "Delete account" in the Clerk dashboard
  (User & Authentication → Settings) so the deletion promise in the policy holds.
...
- In Clerk, enable the sign-up legal-consent checkbox pointing at `/terms` and `/privacy`. PIPL Art. 39
  requires a *separate* affirmative act for cross-border transfer; a dedicated checkbox at sign-up is the
  cleanest evidence of that.
...
- Make the GitHub repository public and enable private vulnerability reporting, as `SECURITY.md` promises.
