...
| 10 | Low | Migration zip step passed export entry names as raw arguments; authenticated GET lacked `Cache-Control`; CSP emitted `upgrade-insecure-requests` in dev (Opus review, findings 7 and 9). | Fixed. |
...
- Deploy scripts refuse to package `.env*`, `.clerk`, `.vercel`, and legacy secret files.
...
- The CSP allows `'unsafe-inline'` for scripts because Next.js emits inline hydration scripts and the theme boot
  script. Moving to nonces requires middleware-generated nonces and is a reasonable follow-up.
...
| 7 | Low | `scripts/migration/remap-convex-export.mjs` | `zip` argument injection via a leading-dash entry name from an untrusted export |
...
- Deploy scripts refuse to package `.env*`, `.clerk`, `.vercel`, and legacy secret files. No env file is tracked. Migration scripts never log keys and use `execFileSync` with argument arrays.
